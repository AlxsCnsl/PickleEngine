...
class NewProject(Script):
    ...
    def _project_exists(project_dir: Path, name_project: str):
        if project_dir.exists():
            print(f"{rich_module(name_project)} already exists in {rich_path(project_dir)}")
            return True
        else:
    ...
    def _make_main_cpp(project_dir: Path, name_project: str):
        main_cpp_path = project_dir / "main.cpp"
        main_cpp_content = \
          "#include <iostream>\n"\
          "\n"\
          "int main() {\n"\
        ...
        print(f"{rich_file('main.cpp')} was created in {rich_path(main_cpp_path)}")
